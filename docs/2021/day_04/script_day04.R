...
#input_numbers <- read_csv("./2021/day_04/test_input_day04.txt", col_names = FALSE) %>% 
input_numbers <- read_csv("./2021/day_04/input_day04.txt", col_names = FALSE) %>% 
  slice(1) %>% 
  mutate(across(everything(), ~ as.numeric(.x))) %>% 
  as_vector()

#boards <- read_table("./2021/day_04/test_input_day04.txt", skip = 2, col_names = FALSE, skip_empty_rows = TRUE) %>% 
